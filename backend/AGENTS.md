...
- **MCP**: served via FastMCP, mounted at `/mcp` in `main.py`; exposes a single curated `search` tool (returns the top 3 hits), decoupled from the REST routes rather than auto-derived from them
