...
import type { NodeMetadata, WorkflowNodeType } from "./NodeMetadata";
...
export class Convert {
  public static toPythonWorkflowDefinition(
    json: string,
  ): PythonWorkflowDefinition {
    return cast(JSON.parse(json), r("PythonWorkflowDefinition"));
  }

  public static pythonWorkflowDefinitionToJson(
    value: PythonWorkflowDefinition,
  ): string {
    return JSON.stringify(
      uncast(value, r("PythonWorkflowDefinition")),
      null,
      2,
    );
  }
...
function convertToNode(
  ...
  if (n.type === Type.Function) {
    ...
    return {
      id: n.id.toString(),
      data: {
        label: n.name ?? metadata.python_import?.split(".").pop() ?? "",
        metadata: metadata,
      },
